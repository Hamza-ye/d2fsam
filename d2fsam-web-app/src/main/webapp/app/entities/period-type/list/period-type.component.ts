...
import { EntityArrayResponseType, PeriodTypeService } from '../service/period-type.service';
...
export class PeriodTypeComponent implements OnInit {
  ...
  trackId = (_index: number, item: IPeriodType): number => this.periodTypeService.getPeriodTypeIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.periodTypes = this.refineData(dataFromBody);
  }

  protected refineData(data: IPeriodType[]): IPeriodType[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IPeriodType[] | null): IPeriodType[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.periodTypeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
