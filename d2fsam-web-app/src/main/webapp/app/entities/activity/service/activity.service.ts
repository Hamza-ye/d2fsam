...
type RestOf<T extends IActivity | NewActivity> = Omit<T, 'created' | 'updated' | 'startDate' | 'endDate'> & {
  created?: string | null;
  updated?: string | null;
...
export class ActivityService {
  ...
  protected convertDateFromClient<T extends IActivity | NewActivity | PartialUpdateActivity>(activity: T): RestOf<T> {
    return {
      ...activity,
      created: activity.created?.toJSON() ?? null,
      updated: activity.updated?.toJSON() ?? null,
      startDate: activity.startDate?.format(DATE_FORMAT) ?? null,
      endDate: activity.endDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restActivity: RestActivity): IActivity {
    return {
      ...restActivity,
      created: restActivity.created ? dayjs(restActivity.created) : undefined,
      updated: restActivity.updated ? dayjs(restActivity.updated) : undefined,
