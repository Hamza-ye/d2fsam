...
type RestOf<T extends ITeamGroup | NewTeamGroup> = Omit<T, 'created' | 'updated' | 'activeFrom' | 'activeTo'> & {
  created?: string | null;
  updated?: string | null;
...
export class TeamGroupService {
  ...
  protected convertDateFromClient<T extends ITeamGroup | NewTeamGroup | PartialUpdateTeamGroup>(teamGroup: T): RestOf<T> {
    return {
      ...teamGroup,
      created: teamGroup.created?.toJSON() ?? null,
      updated: teamGroup.updated?.toJSON() ?? null,
      activeFrom: teamGroup.activeFrom?.toJSON() ?? null,
      activeTo: teamGroup.activeTo?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restTeamGroup: RestTeamGroup): ITeamGroup {
    return {
      ...restTeamGroup,
      created: restTeamGroup.created ? dayjs(restTeamGroup.created) : undefined,
      updated: restTeamGroup.updated ? dayjs(restTeamGroup.updated) : undefined,
      activeFrom: restTeamGroup.activeFrom ? dayjs(restTeamGroup.activeFrom) : undefined,
      activeTo: restTeamGroup.activeTo ? dayjs(restTeamGroup.activeTo) : undefined,
    };
  }
