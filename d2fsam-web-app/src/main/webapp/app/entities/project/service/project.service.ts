...
type RestOf<T extends IProject | NewProject> = Omit<T, 'created' | 'updated'> & {
  created?: string | null;
  updated?: string | null;
};
...
export class ProjectService {
  ...
  protected convertDateFromClient<T extends IProject | NewProject | PartialUpdateProject>(project: T): RestOf<T> {
    return {
      ...
      updated: project.updated?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProject: RestProject): IProject {
    return {
      ...
      updated: restProject.updated ? dayjs(restProject.updated) : undefined,
    };
  }
