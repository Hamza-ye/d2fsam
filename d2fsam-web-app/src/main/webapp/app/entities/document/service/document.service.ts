...
type RestOf<T extends IDocument | NewDocument> = Omit<T, 'created' | 'updated'> & {
  created?: string | null;
  updated?: string | null;
};
...
export class DocumentService {
  ...
  protected convertDateFromClient<T extends IDocument | NewDocument | PartialUpdateDocument>(document: T): RestOf<T> {
    return {
      ...document,
      created: document.created?.toJSON() ?? null,
      updated: document.updated?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDocument: RestDocument): IDocument {
    return {
      ...restDocument,
      created: restDocument.created ? dayjs(restDocument.created) : undefined,
      updated: restDocument.updated ? dayjs(restDocument.updated) : undefined,
    };
  }
