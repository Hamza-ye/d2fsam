...
import { EntityArrayResponseType, RelativePeriodsService } from '../service/relative-periods.service';
...
export class RelativePeriodsComponent implements OnInit {
  ...
  trackId = (_index: number, item: IRelativePeriods): number => this.relativePeriodsService.getRelativePeriodsIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.relativePeriods = this.refineData(dataFromBody);
  }

  protected refineData(data: IRelativePeriods[]): IRelativePeriods[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IRelativePeriods[] | null): IRelativePeriods[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.relativePeriodsService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
