...
import { EntityArrayResponseType, OrganisationUnitLevelService } from '../service/organisation-unit-level.service';
...
export class OrganisationUnitLevelComponent implements OnInit {
  ...
  trackId = (_index: number, item: IOrganisationUnitLevel): number =>
    this.organisationUnitLevelService.getOrganisationUnitLevelIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.organisationUnitLevels = this.refineData(dataFromBody);
  }

  protected refineData(data: IOrganisationUnitLevel[]): IOrganisationUnitLevel[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IOrganisationUnitLevel[] | null): IOrganisationUnitLevel[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.organisationUnitLevelService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
