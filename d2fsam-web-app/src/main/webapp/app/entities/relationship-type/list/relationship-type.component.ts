...
import { EntityArrayResponseType, RelationshipTypeService } from '../service/relationship-type.service';
...
export class RelationshipTypeComponent implements OnInit {
  ...
  trackId = (_index: number, item: IRelationshipType): number => this.relationshipTypeService.getRelationshipTypeIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.relationshipTypes = this.refineData(dataFromBody);
  }

  protected refineData(data: IRelationshipType[]): IRelationshipType[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IRelationshipType[] | null): IRelationshipType[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.relationshipTypeService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
