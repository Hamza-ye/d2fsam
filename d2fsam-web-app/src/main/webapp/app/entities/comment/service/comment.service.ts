...
type RestOf<T extends IComment | NewComment> = Omit<T, 'created' | 'updated'> & {
  created?: string | null;
  updated?: string | null;
};
...
export class CommentService {
  ...
  protected convertDateFromClient<T extends IComment | NewComment | PartialUpdateComment>(comment: T): RestOf<T> {
    return {
      ...comment,
      created: comment.created?.toJSON() ?? null,
      updated: comment.updated?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restComment: RestComment): IComment {
    return {
      ...restComment,
      created: restComment.created ? dayjs(restComment.created) : undefined,
      updated: restComment.updated ? dayjs(restComment.updated) : undefined,
    };
  }
