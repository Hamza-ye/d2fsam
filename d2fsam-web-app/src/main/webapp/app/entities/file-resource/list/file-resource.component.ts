...
import { EntityArrayResponseType, FileResourceService } from '../service/file-resource.service';
...
export class FileResourceComponent implements OnInit {
  ...
  trackId = (_index: number, item: IFileResource): number => this.fileResourceService.getFileResourceIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.fileResources = this.refineData(dataFromBody);
  }

  protected refineData(data: IFileResource[]): IFileResource[] {
    return data.sort(this.sortService.startSort(this.predicate, this.ascending ? 1 : -1));
  }

  protected fillComponentAttributesFromResponseBody(data: IFileResource[] | null): IFileResource[] {
    return data ?? [];
  }
  ...
  protected queryBackend(predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.fileResourceService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
